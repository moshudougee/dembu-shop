...
const Dashboard = () => {
  ...
  const salesData = {
    labels: summary.salesData.map((x: { id: string }) => x.id),
    datasets: [
      {
        fill: true,
        label: 'Sales',
  ...
  const productsData = {
    labels: summary.productsData.map((x: { id: string }) => x.id), // 2022/01 2022/03
    datasets: [
      {
        label: 'Category',
  ...
  const usersData = {
    labels: summary.usersData.map((x: { id: string }) => x.id), // 2022/01 2022/03
    datasets: [
      {
        label: 'Users',
        borderColor: 'rgb(53, 162, 235)',
